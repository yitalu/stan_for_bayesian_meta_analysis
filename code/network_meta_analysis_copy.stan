...
parameters {
    ...
    vector[n_studies_2arms] theta_studies_2arms; // true effect size for each 2-arm study
    // array[n_studies_3arms, 3] real theta_studies_3arms; // true effect size for each 3-arm study
}

transformed parameters {
    matrix[n_treatments, n_treatments] T;
    
    T = rep_matrix(sqrt(tau / 2), n_treatments, n_treatments);
...
model {
    ...
    for (s in 1:n_studies_2arms) {
        ...
        mean_diff_2arms[s] ~ normal(theta_studies_2arms[s], std_err_2arms[s]);

        theta_studies_2arms[s] ~ multi_normal(theta[id_treatments_2arms[s, 1], id_treatments_2arms[s, 2]], T);
